"""Attacker Page — hosts the AttackerPanel and a save-weapon button."""
...
class AttackerPage(customtkinter.CTkFrame):
    ...
    Hosts an AttackerPanel and a button to save the current config as a weapon.
    """
    ...
    def __init__(self, master, state: AppState, **kwargs):
        ...
        # Attacker panel (scrollable form)
        self._panel = AttackerPanel(self, on_change=self._on_panel_change)
        self._panel.grid(row=0, column=0, sticky="nsew", padx=PAGE_PAD_X, pady=(PAGE_PAD_X, 4))

        # Bottom bar with save weapon button
        ...
        # Load current attacker state into panel
